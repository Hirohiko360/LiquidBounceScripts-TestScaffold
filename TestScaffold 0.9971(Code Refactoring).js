...
if (mc.gameSettings.keyBindJump.isKeyDown() && !mc.gameSettings.keyBindLeft.isKeyDown() && !mc.gameSettings.keyBindRight.isKeyDown() && !mc.gameSettings.keyBindForward.isKeyDown() && !mc.gameSettings.keyBindBack.isKeyDown()) {
    ...
    this.onRender3D = function (event) {
		if (settings.scaffoldcircle.get() && circle.scaffoldcircle()) {
            var rangeValue = circle.scaffoldcirclerange().toFixed(2)
            GL11.glPushMatrix();
            GL11.glTranslated(
            ...
            GL11.glLineWidth(1);
            RenderUtils.glColor(new Color(circle.scaffoldcircler(), circle.scaffoldcircleg(), circle.scaffoldcircleb(), circle.scaffoldcircleal()));
            GL11.glRotatef(circle.scaffoldcircleset1(), 1, circle.scaffoldcircleset2(), circle.scaffoldcircleset3());
            GL11.glBegin(GL11.GL_LINE_STRIP);
    
            for (i = 0; i <= circle.scaffoldcircleset4(); i += 60) {
                GL11.glVertex2f(Math.cos(i * Math.PI / circle.scaffoldcircleset5()) * rangeValue, (Math.sin(i * Math.PI / circle.scaffoldcircleset6()) * rangeValue));
            }
...
function circle() {
    ...
    this.getName = function() {
        return "ScaffoldCircle";
    };
    this.getDescription = function() {
        return "ScaffoldCircleAdd";
    };
    this.getCategory = function() {
        return "Fun";
	};
	...
	this.scaffoldcircle = function() {
       return settings.scaffoldcircle.get();
    }
...
function onEnable() {
	...
	circleclient = moduleManager.registerModule(circle);
}

function onDisable() {
	moduleManager.unregisterModule(circle);
