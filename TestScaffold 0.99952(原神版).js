...
function TestScaffold() {
    ...
    var settings = {
        ...
        // Basic stuff
        sprint: setting.boolean("Sprint", false),
		hirohikosprint: setting.boolean("HirohikoSprint", false),
        hypixelsprint: setting.boolean("HypixelSprint", false),
...
function add3() {
	...
	this.onRender2D = function(event) {
		...
		GL11.glPopMatrix();
		Fonts.font40.drawStringWithShadow("你说得对,但是感觉不如原神",mcWidth/2/2+settings.psx.get()+18, mcHeight/2 - 20 + settings.psy.get(),textcolor);
		Fonts.font40.drawStringWithShadow("我们这个压缩毛巾体积小方便携带，拆开一包，放水里就变大，怎么扯都扯不坏",mcWidth/2/2+settings.psx.get(), mcHeight/2 - 9 + settings.psy.get(),textcolor);
		Fonts.font40.drawStringWithShadow("用来擦脚，擦脸，擦嘴都是很好用的，你看打开以后像圆饼一样大小",mcWidth/2/2+settings.psx.get(), mcHeight/2 + settings.psy.get(),textcolor);
		Fonts.font40.drawStringWithShadow("放在水里遇水变大变高，吸水性很强的。打开以后是一条加大加厚的毛巾",mcWidth/2/2+settings.psx.get(), mcHeight/2 + 9 + settings.psy.get(),textcolor);
		Fonts.font40.drawStringWithShadow("你看他怎么挣都挣不坏",mcWidth/2/2+settings.psx.get(), mcHeight/2 + 18 + settings.psy.get(),textcolor);
		Fonts.font40.drawStringWithShadow("好不掉毛不掉絮，使用七八次都没问题，出差旅行带上它非常方便，用它擦擦脚",mcWidth/2/2+settings.psx.get(), mcHeight/2 + 27 + settings.psy.get(),textcolor);
		Fonts.font40.drawStringWithShadow("再擦擦嘴，擦擦脸，干净卫生",mcWidth/2/2+settings.psx.get(), mcHeight/2 + 36 + settings.psy.get(),textcolor);
		Fonts.font40.drawStringWithShadow("什么?在哪里买?下方小黄车，买五包送五包，还包邮。",mcWidth/2/2+settings.psx.get(), mcHeight/2 + 45 + settings.psy.get(),textcolor);
	}
