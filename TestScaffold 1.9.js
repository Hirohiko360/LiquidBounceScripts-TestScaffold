...
var colorchoose = ["§1", "§2", "§3", "§4", "§5", "§6", "§7", "§8", "§9", "§0", "§a", "§b", "§c", "§d", "§e", "§f", "§k", "§l", "§o", "§m", "§n"];
var colorrandom = Math.floor(Math.random() * 20) + 1;
var colorset = colorchoose[colorrandom]

var scriptName = colorset + "TestScaffold";
var scriptVersion = colorset + 1.0;
var scriptAuthor = colorset + "mumy && yby360";
...
// Classification List
var setchoose = ["Skip", "Custom"];
var libchoose = ["External resources", "Custom"];
var listshowchoose = ["push-button", "tabulation"];
var choose = ["World", "Fun", "Movement", "Render", "Combat", "Player", "Misc", "Exploit", "Custom"];

var setset = JOptionPane.showOptionDialog(null,"Compatibility Settings","compatibility",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE,null,setchoose,setchoose[0]);
var listsetset = setchoose[setset]

if(listsetset == "Custom") {

var libset = JOptionPane.showInputDialog(null,"Please select an API loading location：","API Load",JOptionPane.QUESTION_MESSAGE,null,libchoose,libchoose[0]);
	if (libset == "External resources") {
		script.import("lib/minecraftUtils.js");
		script.import("lib/glFunctions.js");
		script.import("lib/systemFunctions.js")
		} 
	else {
			var libfst = JOptionPane.showInputDialog(null,"Please enter the first custom library name：","Custom lib",JOptionPane.WARNING_MESSAGE);
			var libsed = JOptionPane.showInputDialog(null,"Please enter a second custom library name：","Custom lib",JOptionPane.WARNING_MESSAGE);
			var libend = JOptionPane.showInputDialog(null,"Please enter a third custom library name：","Custom lib",JOptionPane.WARNING_MESSAGE);
			if (libfst=="" && libsed=="" && libend=="") {
					JOptionPane.showMessageDialog(null,"Input cannot be empty!","Custom lib",JOptionPane.WARNING_MESSAGE);
				}
			script.import("lib/"+libfst+".js")
			script.import("lib/"+libsed+".js")
			script.import("lib/"+libend+".js")
		}
				
var listshowset = JOptionPane.showOptionDialog(null,"Please select a presentation form：","Presentation form",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE,null,listshowchoose,listshowchoose[0]);
var listshow = listshowchoose[listshowset]

if(listshow == "push-button") {
	var chooseset = JOptionPane.showOptionDialog(null,"Please select a classification：","classify",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE,null,choose,choose[0]);
	var chooselist = choose[chooseset]
		if(chooselist == "Custom") {
			var chooselist = JOptionPane.showInputDialog(null,"Please enter a custom name：","The first letter should be capitalized",JOptionPane.WARNING_MESSAGE);	
				}
					}else{
						var chooselist = JOptionPane.showInputDialog(null,"Please select a classification：","classify",JOptionPane.QUESTION_MESSAGE,null,choose,choose[0]);	
							if(chooselist == "Custom") {
								var chooselist = JOptionPane.showInputDialog(null,"Please enter a custom name：","The first letter should be capitalized",JOptionPane.WARNING_MESSAGE);
							}
						}
...
function TestScaffold() {
    ...
    var settings = {
        // Mode
        mode: setting.list(colorset + "Mode", ["Normal", "Rewinside", "Expand", "MatrixTest", "MLG"], "Normal"),
		rewinsidestrafe: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.mode.get().toLowerCase() === "rewinside"
			}
		}))(colorset + "RewinsideStrafe", 0.2, 0.1, 0.5)),
		rewinsidemotiony: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.mode.get().toLowerCase() === "rewinside"
			}
		}))(colorset + "RewinsideMotionY", 0, 0, 0.5)),
        ...
        }))(colorset + "MinDelay", 0, 0, 1000)),
        placeableDelay: setting.boolean(colorset + "PlaceableDelay", false),

        // AutoBlock
		autoBlock: setting.list(colorset + "AutoBlockMode", ["Off", "Test", "Pick", "Spoof"], "Off"),
        stayAutoBlock: setting.boolean(colorset + "StayAutoBlock", false),

        // Basic stuff
        sprint: setting.list(colorset + "SprintMode", ["Off", "Normal", "Hypixel", "Fake"], "Off"),
		hypixelsprintspeed: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.sprint.get().toLowerCase() === "hypixel"
			}
		}))(colorset + "HypixelSprintSpeed", 1.2, 1, 1.5)),
		hirohikosprint: setting.boolean(colorset + "HirohikoSprint", false),
        swing: setting.list(colorset + "SwingMode", ["Off", "OffPacket", "Normal"], "Off"),
        search: setting.boolean(colorset + "Search", true),
        down: setting.boolean(colorset + "Down", true),
        placeMode: setting.list(colorset + "PlaceTiming", ["Pre", "Post"], "Post"),

        // Eagle
        eagle: setting.boolean(colorset + "Eagle", false),
		eagleSilent: new _AdaptedValue(new (Java.extend(BoolValue, {
		isSupported: function () {
			return settings.eagle.get()
			}
		}))(colorset + "EagleSilent", false)),
		blocksToEagle: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.eagle.get()
			}
		}))(colorset + "BlocksToEagle", 0, 0, 10)),

        // Expand
		expandLength: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.mode.get().toLowerCase() === "expand"
			}
		}))(colorset + "ExpandLength", 5, 1, 6)),

        // Rotations
        rotations: setting.boolean(colorset + "Rotations", true),
		rotationsadd: setting.list(colorset + "RotationsModeAdd", ["Off", "Test", "CustomYaw-Offset-Head"], "Off"),
		horse: new _AdaptedValue(new (Java.extend(BoolValue, {
		isSupported: function () {
			return settings.rotationsadd.get().toLowerCase() === "test"
			}
		}))(colorset + "TestRotations-IfHorse", true)),
		rotationYaw: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.rotationsadd.get().toLowerCase() === "customyaw-offset-head"
			}
		}))(colorset + "RotationYaw", 90, 0, 360)),
				rotationPitchHead: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.rotationsadd.get().toLowerCase() === "customyaw-offset-head"
			}
		}))(colorset + "RotationPitchHead", 90, 0, 360)),
				renderYawOffset: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.rotationsadd.get().toLowerCase() === "customyaw-offset-head"
			}
		}))(colorset + "RenderYawOffset", 90, 0, 360)),
				rotationYawHead: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.rotationsadd.get().toLowerCase() === "customyaw-offset-head"
			}
		}))(colorset + "RotationYawHead", 90, 0, 360)),
				ridingEntity: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.rotationsadd.get().toLowerCase() === "customyaw-offset-head"
			}
		}))(colorset + "RidingEntity", 90, 0, 360)),
		keepRotation: setting.boolean(colorset + "KeepRotation", false),
		keepLength: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.keepRotation.get()
			}
		}))(colorset + "KeepRotationLength", 0, 0, 20)),
        
        // Zitter
        zitterMode: setting.list(colorset + "ZitterMode", ["Off", "Teleport", "Smooth"], "Teleport"),
		zitterSpeed: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.zitterMode.get().toLowerCase() != "off"
			}
		}))(colorset + "ZitterSpeed", 0.13, 0.1, 0.3)),
		zitterStrength: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.zitterMode.get().toLowerCase() != "off"
			}
		}))(colorset + "ZitterStrength", 0.072, 0.05, 0.2)),

        // Game
		timermode: setting.list(colorset + "TimerMode", ["Normal", "Gradual"], "Normal"),
		timer: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.timermode.get().toLowerCase() === "normal"
			}
		}))(colorset + "NormalTimer", 1, 0.1, 10)),
		startTimer: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.timermode.get().toLowerCase() === "gradual"
			}
		}))(colorset + "GradualStartTimer", 1, 0.1, 10)),
		endTimer: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.timermode.get().toLowerCase() === "gradual"
			}
		}))(colorset + "GradualEndTimer", 1, 0.1, 10)),
		Timerspeed: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.timermode.get().toLowerCase() === "gradual"
			}
		}))(colorset + "GradualTimerspeed",20,1,20)),
		Timerdelay: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.timermode.get().toLowerCase() === "gradual"
			}
		}))(colorset + "GradualTimerdelay",20,1,20)),
        speedModifier: setting.float(colorset + "SpeedModifier", 1, 0, 2),
		slow: setting.boolean(colorset + "Slow", false),
		slowspeed: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.slow.get()
			}
		}))(colorset + "SlowSpeed", 0.6, 0.2, 0.8)),

        // Safety
        antihalf: setting.boolean(colorset + "AntiHalf", false),
        bodyreverser: setting.boolean(colorset + "BodyReverser", false),
		AntiFlag: setting.boolean(colorset + "AntiFlag", false),
		AntiFlagActivation: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.AntiFlag.get()
			}
		}))(colorset + "AntiFlagActivation",20,1,100)),
		AntiFlagStrength: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.AntiFlag.get()
			}
		}))(colorset + "AntiFlagStrength",0.01,0.05,0.30)),
        autowalkmode: setting.list(colorset + "AutoWalkMode", ["Off", "Test", "Bind"], "Test"),
        sameY: setting.boolean(colorset + "SameY", false),
        safeWalk: setting.boolean(colorset + "SafeWalk", true),
        airSafe: setting.boolean(colorset + "AirSafe", false),
		maxfalldistance: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.airSafe.get()
			}
		}))(colorset + "AirSafeMaxFallDistance", 5, 0, 255)),
		
		// Module+
        buildHand: setting.boolean(colorset + "BuildHand", false),
        towermode: setting.list(colorset + "TowerMode", ["Off", "TestTower", "TestTower2", "BindTower", "AACTower", "MotionTp2", "AAC4Tower", "VerusTower", "AAC4JumpTower", "Matrix6.9.2Tower", "UniversoCraftTower", "Hiro", "CustomTower"], "TestTower"),
        motionground: setting.boolean(colorset + "MotionGround", false),
		motiongroundset: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.motionground.get()
			}
		}))(colorset + "MotionGroundSet", 2.5, 0, 3)),
        speed: setting.boolean(colorset + "Speed", false),
		speedmode: new _AdaptedValue(new (Java.extend(ListValue, {
		isSupported: function () {
			return settings.speed.get()
			}
		}))(colorset + "SpeedMode", ["AutoJump", "BindSpeed", "TestSpeed", "TestSpeed2", "TestHop", "AACTestSpeed", "AACTestSpeed2", "HClip", "CustomBHop", "MatrixHop", "Vulcan", "Ncp", "Verus", "KarhuLow", "BlocksMC", "Spartan", "NoRules", "Dev"], "AutoJump")),
        xzrjump: setting.boolean(colorset + "XZRJump", false),
		xzrjumpset: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.xzrjump.get()
			}
		}))(colorset + "XZRJumpSet", 0, 0, 0.02)),
        vclipjump: setting.boolean(colorset + "VClipJump", false),
		vclipjumpset: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.vclipjump.get()
			}
		}))(colorset + "VClipJumpSet", 0.1, 0.01, 1)),
		randomjump: setting.boolean(colorset + "RandomJump", false),
		randomjumpmax: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.randomjump.get()
			}
		}))(colorset + "RandomJumpMax", 0.84, 0.01, 1)),
		randomjumpmin: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.randomjump.get()
			}
		}))(colorset + "RandomJumpMin", 0.01, 0.01, 1)),
		fusrodah: setting.boolean(colorset + "FusRoDah", false),
		fusrodahset: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.fusrodah.get()
			}
		}))(colorset + "FusRoDahSet", 0.5, 0.1, 1.5)),
		fusrodahhurtset: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.fusrodah.get()
			}
		}))(colorset + "FusRoDahHurtSet", 0, 0, 20)),
		blink: setting.boolean(colorset + "Blink", false),
		customdisabler: setting.boolean(colorset + "CustomDisabler", false),
		killauraswitch: setting.boolean(colorset + "KillAuraSwitch", false),
		buildtargetstrafe: setting.boolean(colorset + "BuildTargetStrafe", false),
		distance0: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.buildtargetstrafe.get()
			}
		}))(colorset + "Distance", 2.5, 1.0, 4.00)),
		motionXZ: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.buildtargetstrafe.get()
			}
		}))(colorset + "MotionXZ" , 0.28, 0.01, 0.60)),
		
		// RepairFix
		DisableHeldItemChange: setting.boolean(colorset + "DisableHeldItemChange", false),
		
        // Visuals
        counterDisplay: setting.boolean(colorset + "Counter", true),
        smoothcamera: setting.boolean(colorset + "SmoothCamera", false),
        skytime: setting.boolean(colorset + "SkyTime", false),
		skyTimeAmount: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.skytime.get()
			}
		}))(colorset + "SkyTimeAmount", 50, -100, 100)),
        Heartbeat: setting.boolean(colorset + "HeartBeat", false),
		Heartbeatset: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.Heartbeat.get()
			}
		}))(colorset + "HeartBeatSet", 90, 0, 360)),
        mark: setting.boolean(colorset + "Mark", false),
		scaffoldcircle: setting.boolean(colorset + "ScaffoldCircle", false),
		speedmonitor: setting.boolean(colorset + "SpeedMonitor", false),
		thirdPersonView: setting.boolean(colorset + "ThirdPersonView",false),
		hideGUI: setting.boolean(colorset + "hideGUI", false),
		
		// FunAdd
		deathcoords: setting.boolean(colorset + "DeathCoords", false),
		debugcam: setting.boolean(colorset + "DeBugCam", false),
		mousesensitivity: setting.boolean(colorset + "MouseSensitivity", false),
		mousesensitivityset: new _AdaptedValue(new (Java.extend(FloatValue, {
		isSupported: function () {
			return settings.mousesensitivity.get()
			}
		}))(colorset + "MouseSensitivitySet", 0.5, 0.1, 1.5)),
		memoryback: setting.boolean(colorset + "MemoryBack", false),
		memorybacktime: new _AdaptedValue(new (Java.extend(IntegerValue, {
		isSupported: function () {
			return settings.memoryback.get()
			}
		}))(colorset + "MemoryBackTime", 3000, 1000, 100000)),
		betterfps: setting.boolean(colorset + "BetterFps", false)
	...
	this.links23 = function() {
      return settings.towermode.get().toLowerCase() === "customtower";
    }
    ...
    this.getDescription = function () {
        return colorset + "TestScaffold-Module, By yby360 and mumy";
    }
	...
	this.getTag = function () {
        return colorset + settings.mode.get();
    }
...
this.onPacket = function (event) {
	...
	switch (settings.rotationsadd.get()) {
		case "Off":
		break;
		case "Test":
		...
		break;
		case "CustomYaw-Offset-Head":
		mc.thePlayer.rotationYaw = settings.rotationYaw.get(); 
		mc.thePlayer.rotationPitchHead = settings.rotationPitchHead.get(); 
		mc.thePlayer.renderYawOffset = mc.thePlayer.rotationYaw + settings.renderYawOffset.get();
		mc.thePlayer.rotationYawHead = mc.thePlayer.rotationYaw + settings.rotationYawHead.get();
		mc.thePlayer.ridingEntity = settings.ridingEntity.get();
		break;	
		}
	}
...
if (mc.thePlayer.onGround) {
	...
	case "Hiro":
		if (mc.thePlayer.ticksExisted % 4 == 1) {
                ...
                jumpGround = mc.thePlayer.posY
                mc.thePlayer.setPosition(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ)
                if (mc.thePlayer.posY > jumpGround) {
                    mc.thePlayer.jump();
                    mc.thePlayer.setPosition(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ)
                    jumpGround = mc.thePlayer.posY
                }
...
if (settings.mode.get().toLowerCase() === "mlg") {
...
if (settings.buildHand.get()) {
    ...
    function update() {
        ...
        if (settings.autoBlock.get().toLowerCase() !="off" ? InventoryUtils.findAutoBlockBlock() === -1 && !isHeldItemBlock : !isHeldItemBlock) {
            return;
        }
...
if (mc.gameSettings.keyBindJump.isKeyDown() && !mc.gameSettings.keyBindLeft.isKeyDown() && !mc.gameSettings.keyBindRight.isKeyDown() && !mc.gameSettings.keyBindForward.isKeyDown() && !mc.gameSettings.keyBindBack.isKeyDown()) {
    ...
    this.onDisable = function () {
		...
		// Autowalk_False
		if (settings.autowalkmode.get().toLowerCase() !="off"){
		autowalk.setState(false)
		mc.gameSettings.keyBindForward.pressed = false;
		}
...
function add3() {
	...
	var settings = {
		guiadd: setting.boolean("GuiAdd",false),
		...
		CommandSystemMode: setting.list("CommandSystemMode",["Off","TestScaffold","TestScaffoldAdd","TestScaffoldAdd2","TestScaffoldAddGui"],"TestScaffold"),
		 };
	...
	if(TestScaffold.links6().toLowerCase() !="off"){
			colorb = "Enabled";
