...
function TestScaffold() {
    ...
    var settings = {
        ...
        // AutoBlock
		autoBlock: setting.list("AutoBlockMode", ["Off", "Test", "Pick", "Spoof"], "Off"),
		...
		rotationsadd: setting.list("RotationsModeAdd", ["Off", "Test", "CustomYaw-Offset-Head"], "Off"),
		horse: new _AdaptedValue(new (Java.extend(BoolValue, {
		isSupported: function () {
			return settings.rotationsadd.get().toLowerCase() === "test"
			}
		}))("TestRotations-IfHorse", true)),
		...
		}))("AirSafeMaxFallDistance", 5, 0, 255)),
		
		// Module+
        buildHand: setting.boolean("BuildHand", false),
        towermode: setting.list("TowerMode", ["Off", "TestTower", "TestTower2", "BindTower", "AACTower", "MotionTp2", "AAC4Tower", "VerusTower", "AAC4JumpTower", "Matrix6.9.2Tower", "UniversoCraftTower", "Hiro", "CustomTower"], "TestTower"),
		...
		}))("MotionGroundSet", 2.5, 0, 3)),
        speed: setting.boolean("Speed", false),
		speedmode: new _AdaptedValue(new (Java.extend(ListValue, {
		isSupported: function () {
			return settings.speed.get()
			}
		}))("SpeedMode", ["AutoJump", "BindSpeed", "TestSpeed", "TestSpeed2", "TestHop", "AACTestSpeed", "AACTestSpeed2", "HClip", "CustomBHop", "MatrixHop", "Vulcan", "Ncp", "Verus", "KarhuLow", "BlocksMC", "Spartan", "NoRules", "Dev"], "AutoJump")),
		...
		}))("MouseSensitivitySet", 0.5, 0.1, 1.5)),
		...
		}))("MemoryBackTime", 3000, 1000, 100000)),
...
function add5() {
 ...
 PacketBuffer: setting.boolean("PacketBuffer", false),
	...
	this.onEnable = function() {
		Chat.print("名称以 C 开头的数据包是客户端数据包，以 S 开头的数据包是服务器数据包");
	}
...
function onEnable() {
	...
	add5client = moduleManager.registerModule(add5);
	CommandSystemInstance.register();
}

function onDisable() {
	CommandSystemInstance.unregister();
	moduleManager.unregisterModule(add5);
